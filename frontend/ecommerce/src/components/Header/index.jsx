import logo from "../../assets/img/logo.png";
import styles from "./index.module.css";
import Cart from './Cart'
import { Link } from "react-router-dom";
import { UserContext } from "../../App";
import { useContext } from "react";


function Header() {
  const userContext = useContext(UserContext);
  let isLogin=(userContext[0]!=null);
  let name=(isLogin)?(JSON.parse(userContext[0])?.name):null;
  function handleLogout(){
    localStorage.clear();
    userContext[1](null);
  }
  return (
    <header>
      <div className="container p-0">
        <div className="row">
          <div className="col-lg-2 col-xl-3 col-md-3 col-6">
            <a href="http://localhost:3000/">
              <img width="170" height="43" src={logo} alt="" />
            </a>
          </div>
          <div className="col-lg-8 col-xl-7 col-md-6 col-10 d-flex align-items-center">
            <div className={`${styles["theme-search"]} me-2 border-1 d-flex align-items-center`}>
              <input type="text" name="" id="" className="border-0 pt-1" placeholder="Tìm kiếm" />
              <i className="fa fa-search" aria-hidden="true" style={{ "fontSize": "23px", cursor:"pointer", opacity:"0.7"}}></i>
            </div>
            <div className={`${styles["contact-phone"]} mx-3 d-flex align-items-center`}>
              <svg width="35" height="35" viewBox="0 0 74 74" xmlns="http://www.w3.org/2000/svg" >
                <g>
                  <path xmlns="http://www.w3.org/2000/svg" d="m55.433 72.005a29.212 29.212 0 0 1 -10.049-2.079c-8.05-3.008-16.773-8.956-24.563-16.747s-13.739-16.513-16.747-24.563c-3.105-8.306-2.722-14.973 1.078-18.773l7.2-7.2a2.2 2.2 0 0 1 3.107 0l13.551 13.552a2.2 2.2 0 0 1 0 3.108l-5.505 5.5c-1.334 1.334-1.394 3.989-.163 7.283a32.567 32.567 0 0 0 7.537 11.032 32.567 32.567 0 0 0 11.031 7.541c3.294 1.23 5.948 1.171 7.283-.163l5.5-5.505a2.251 2.251 0 0 1 3.108 0l13.555 13.551a2.2 2.2 0 0 1 0 3.107l-7.2 7.2c-2.099 2.1-5.077 3.156-8.723 3.156zm-41.533-68.005a.2.2 0 0 0 -.141.058l-7.2 7.2c-6.849 6.849.18 25.021 15.668 40.509s33.673 22.515 40.516 15.667l7.2-7.2a.2.2 0 0 0 0-.28l-13.552-13.554a.2.2 0 0 0 -.28 0l-5.5 5.505c-1.94 1.941-5.277 2.162-9.4.623a34.577 34.577 0 0 1 -11.746-8 34.577 34.577 0 0 1 -8-11.746c-1.539-4.119-1.318-7.457.623-9.4l5.505-5.5a.2.2 0 0 0 0-.28l-13.549-13.544a.2.2 0 0 0 -.144-.058z" fill="#ed1f27" data-original="#000000" ></path>
                  <path xmlns="http://www.w3.org/2000/svg" d="m22.8 25.1a1 1 0 0 1 -.707-.293l-15.247-15.244a1 1 0 0 1 1.414-1.414l15.24 15.245a1 1 0 0 1 -.7 1.706z" fill="#ed1f27" data-original="#000000"></path>
                  <path xmlns="http://www.w3.org/2000/svg" d="m65.144 67.447a1 1 0 0 1 -.707-.293l-15.245-15.244a1 1 0 0 1 1.414-1.41l15.245 15.24a1 1 0 0 1 -.707 1.707z" fill="#ed1f27" data-original="#000000" ></path>
                  <path xmlns="http://www.w3.org/2000/svg" d="m70.5 35.5a1 1 0 0 1 -1-1 30.034 30.034 0 0 0 -30-30 1 1 0 0 1 0-2 32.036 32.036 0 0 1 32 32 1 1 0 0 1 -1 1z" fill="#ed1f27" data-original="#000000"></path>
                  <path xmlns="http://www.w3.org/2000/svg" d="m60.961 35.5a1 1 0 0 1 -1-1 20.485 20.485 0 0 0 -20.461-20.461 1 1 0 0 1 0-2 22.487 22.487 0 0 1 22.461 22.461 1 1 0 0 1 -1 1z" fill="#ed1f27" data-original="#000000" ></path>
                  <path xmlns="http://www.w3.org/2000/svg" d="m51.423 35.5a1 1 0 0 1 -1-1 10.935 10.935 0 0 0 -10.923-10.923 1 1 0 0 1 0-2 12.938 12.938 0 0 1 12.923 12.923 1 1 0 0 1 -1 1z" fill="#ed1f27" data-original="#000000"></path>
                </g>
              </svg>
              <div className="ms-2">
                <p className="m-0 pt-1">Hotline</p>
                <a className={styles.phoneNumber} href="tel:+84938150517">0938 150 517</a>
              </div>
              
            </div>
            <div className="account d-flex align-items-center">
            <svg width="35" height="35" enableBackground="new 0 0 512 512" version="1.1" viewBox="0 0 480 480" xmlns="http://www.w3.org/2000/svg">
              <g xmlns="http://www.w3.org/2000/svg">
                <path d="m240 0c-132.34 0-240 107.66-240 240 0 57.96 20.656 111.18 54.992 152.7 0.088 0.12 0.096 0.272 0.192 0.384 24.792 29.896 55.928 52.816 90.624 67.624 0.4 0.168 0.792 0.352 1.192 0.52 2.808 1.184 5.648 2.28 8.496 3.352 1.12 0.424 2.24 0.856 3.376 1.264 2.456 0.88 4.928 1.712 7.416 2.512 1.592 0.512 3.184 1.016 4.792 1.496 2.2 0.656 4.408 1.288 6.632 1.888 1.952 0.528 3.92 1.016 5.888 1.488 1.992 0.48 3.992 0.96 6 1.384 2.24 0.48 4.504 0.904 6.776 1.32 1.824 0.336 3.64 0.688 5.48 0.984 2.52 0.408 5.056 0.728 7.6 1.056 1.64 0.208 3.272 0.448 4.92 0.624 2.88 0.304 5.784 0.52 8.696 0.72 1.352 0.096 2.696 0.24 4.056 0.312 4.248 0.24 8.544 0.368 12.872 0.368s8.624-0.128 12.888-0.352c1.36-0.072 2.704-0.216 4.056-0.312 2.912-0.208 5.816-0.416 8.696-0.72 1.648-0.176 3.28-0.416 4.92-0.624 2.544-0.328 5.08-0.648 7.6-1.056 1.832-0.296 3.656-0.648 5.48-0.984 2.264-0.416 4.528-0.84 6.776-1.32 2.008-0.432 4-0.904 6-1.384 1.968-0.48 3.936-0.968 5.888-1.488 2.224-0.592 4.432-1.232 6.632-1.888 1.608-0.48 3.2-0.984 4.792-1.496 2.488-0.8 4.96-1.632 7.416-2.512 1.128-0.408 2.248-0.84 3.376-1.264 2.856-1.072 5.688-2.176 8.496-3.352 0.4-0.168 0.792-0.352 1.192-0.52 34.688-14.808 65.832-37.728 90.624-67.624 0.096-0.112 0.104-0.272 0.192-0.384 34.32-41.536 54.976-94.76 54.976-152.72 0-132.34-107.66-240-240-240zm97.256 441.76c-0.12 0.056-0.232 0.12-0.352 0.176-2.856 1.376-5.76 2.672-8.688 3.936-0.664 0.28-1.32 0.568-1.984 0.848-2.56 1.072-5.152 2.088-7.76 3.064-1.088 0.408-2.176 0.808-3.272 1.192-2.312 0.824-4.632 1.616-6.976 2.368-1.456 0.464-2.92 0.904-4.384 1.336-2.08 0.624-4.168 1.224-6.28 1.784-1.776 0.472-3.568 0.904-5.36 1.328-1.88 0.448-3.752 0.904-5.648 1.304-2.072 0.44-4.16 0.816-6.24 1.192-1.688 0.312-3.368 0.64-5.072 0.912-2.344 0.368-4.712 0.664-7.072 0.96-1.496 0.192-2.984 0.416-4.496 0.576-2.696 0.288-5.416 0.472-8.128 0.664-1.208 0.08-2.408 0.216-3.632 0.28-3.96 0.208-7.928 0.32-11.912 0.32s-7.952-0.112-11.904-0.32c-1.216-0.064-2.416-0.192-3.632-0.28-2.72-0.184-5.432-0.376-8.128-0.664-1.512-0.16-3-0.384-4.496-0.576-2.36-0.296-4.728-0.592-7.072-0.96-1.704-0.272-3.384-0.6-5.072-0.912-2.088-0.376-4.176-0.76-6.24-1.192-1.896-0.4-3.776-0.856-5.648-1.304-1.792-0.432-3.584-0.856-5.36-1.328-2.104-0.56-4.2-1.168-6.28-1.784-1.464-0.432-2.928-0.872-4.384-1.336-2.344-0.752-4.672-1.544-6.976-2.368-1.096-0.392-2.184-0.792-3.272-1.192-2.608-0.976-5.2-1.992-7.76-3.064-0.664-0.272-1.312-0.56-1.976-0.84-2.928-1.256-5.832-2.56-8.696-3.936-0.12-0.056-0.232-0.112-0.352-0.176-27.912-13.504-52.568-32.672-72.576-55.952 15.464-56.944 59.24-102.85 115.56-121.11 1.112 0.68 2.272 1.288 3.416 1.928 0.672 0.376 1.336 0.776 2.016 1.136 2.384 1.264 4.8 2.448 7.272 3.512 1.896 0.832 3.856 1.536 5.808 2.256 0.384 0.136 0.768 0.288 1.152 0.424 10.848 3.84 22.456 6.04 34.6 6.04s23.752-2.2 34.592-6.04c0.384-0.136 0.768-0.288 1.152-0.424 1.952-0.72 3.912-1.424 5.808-2.256 2.472-1.064 4.888-2.248 7.272-3.512 0.68-0.368 1.344-0.76 2.016-1.136 1.144-0.64 2.312-1.248 3.432-1.936 56.32 18.272 100.09 64.176 115.56 121.11-20.008 23.272-44.664 42.44-72.576 55.952zm-185.26-265.76c0-48.52 39.48-88 88-88s88 39.48 88 88c0 30.864-16.008 58.024-40.128 73.736-3.152 2.048-6.432 3.88-9.8 5.48-0.4 0.192-0.792 0.392-1.192 0.576-23.168 10.536-50.592 10.536-73.76 0-0.4-0.184-0.8-0.384-1.192-0.576-3.376-1.6-6.648-3.432-9.8-5.48-24.12-15.712-40.128-42.872-40.128-73.736zm269.83 194.58c-18.136-53.552-59.512-96.832-112.38-117.39 21.144-19.048 34.544-46.552 34.544-77.192 0-57.344-46.656-104-104-104s-104 46.656-104 104c0 30.64 13.4 58.144 34.552 77.192-52.864 20.568-94.24 63.84-112.38 117.39-26.504-36.792-42.176-81.88-42.176-130.58 0-123.51 100.49-224 224-224s224 100.49 224 224c0 48.704-15.672 93.792-42.168 130.58z" fill="#ef0000" data-original="#000000" />
              </g>
            </svg>
              <div className="ms-2">
                <p className="m-0">{(isLogin)?name:"Tài khoản"}</p>
                {(isLogin)?
                  <Link className={styles.login} to="/" onClick={handleLogout}>Thoát</Link>:
                  <Link className={styles.login} to="/account">Đăng nhập</Link>
                }
              </div>
            </div>
          </div>
          <div className="col-lg-2 col-md-3 col-6">
            <div className="group-account h-100 d-flex align-items-center justify-content-center">
              <div className={styles.icon}>
                <Link to="/wishlist">
                  <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" className="bi bi-heart" viewBox="0 0 16 16">
                    <path d="m8 2.748-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143c.06.055.119.112.176.171a3.12 3.12 0 0 1 .176-.17C12.72-3.042 23.333 4.867 8 15z"/>
                  </svg>
                </Link>
              </div>
              <div className={styles.icon}>
                <Link to="/cart">
                  <svg width="25" height="25" viewBox="0 -45 419.24 419" xmlns="http://www.w3.org/2000/svg">
                    <path d="m359.08 102.91c1.4609 1.5391 3.4766 2.4375 5.6016 2.4844 2.125 0.050781 4.1797-0.75 5.7109-2.2227l46.414-44.898c1.5625-1.5117 2.4414-3.5938 2.4336-5.7656-0.003906-2.1758-0.89453-4.2539-2.4688-5.7539l-46.41-44.41c-3.1953-3.0547-8.2578-2.9414-11.312 0.25-1.4648 1.4922-2.2656 3.5156-2.2188 5.6055 0.046875 2.0898 0.9375 4.0742 2.4688 5.5l32.008 30.426h-64.273c-27.59 0.14453-53.539 13.133-70.191 35.137l-54.852 71.617-79.242-103.46c-1.4961-2.0156-3.8359-3.2305-6.3477-3.2891h-108.4c-4.418 0-8 3.582-8 8 0 4.418 3.582 8 8 8h104.45l79.449 104-79.449 104h-104.45c-4.418 0-8 3.582-8 8 0 4.418 3.582 8 8 8h108.4c2.5195-0.078125 4.8594-1.3086 6.3477-3.3359l79.242-103.41 54.812 71.617c16.66 22.012 42.625 35.004 70.23 35.133h64.273l-32.008 30.426c-2.3555 2.2344-3.1016 5.6875-1.8789 8.6992 1.2227 3.0078 4.1602 4.9648 7.4102 4.9258 2.0625-0.007813 4.043-0.8125 5.5312-2.2461l46.41-44.426c1.5742-1.5039 2.4648-3.5859 2.4688-5.7617 0.007813-2.1758-0.87109-4.2617-2.4336-5.7773l-46.414-44.91c-3.1758-3.0742-8.2422-2.9883-11.312 0.18359-3.0703 3.2539-2.9883 8.3633 0.1875 11.516l32.207 31.371h-64.441c-22.617-0.14062-43.875-10.816-57.5-28.875l-57.461-75.023 57.5-75.227c13.613-18.043 34.855-28.719 57.461-28.875h64.441l-32.207 31.371c-3.1758 3.1094-3.2578 8.1992-0.1875 11.414z"></path>
                  </svg>
                </Link>
              </div>
              <Cart />
            </div>
          </div>
        </div>
      </div>
    </header>
  );
}
export default Header;
